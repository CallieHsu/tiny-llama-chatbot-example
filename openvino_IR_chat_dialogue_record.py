...
def truncate(dialogue_history, limit=400):
    total = 0
    truncate_dialogue = list()
    for msg in reversed(dialogue_history[1:]):
        total += len(tokenizer(msg["content"], return_tensors="pt").input_ids[0])
        if total > limit:
            break
        truncate_dialogue.insert(0, msg)
    truncate_dialogue.insert(0, dialogue_history[0])
    return truncate_dialogue

def chat(dialogue_history):
    input_text = tokenizer.apply_chat_template(dialogue_history, tokenize=True, add_generation_prompt=True, return_tensors="pt")
...
while True:
    ...
    dialogue_history = truncate(dialogue_history)
    print("Model:")
    response = chat(dialogue_history)

    model_reply = tokenizer.decode(response[0], skip_special_tokens=True).split("<|assistant|>")[-1].strip()
    dialogue_history.append({"role": "assistant", "content": model_reply})
